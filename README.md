...
Wave functions can be specified using Lua expressions. These expressions can use
two variables: `position` is a normalised wavetable index, which varies from 0.0
when generating the first waveform in the table to 1.0 when generating the last
wave; `theta` varies between 0 and 2 PI along the length of each waveform.
